...
def printInfo():
    tk.messagebox.showinfo(title="Digitera", message="Digitera - Created by Lucas Meira during COVID-19 Quarantine, April 15th 2020")

def printGuide():
    tk.messagebox.showinfo(title="Digitera", message="Thank you for using Digitera!\nPlease Refer to https://github.com/Lucas-Meira/MNIST-Digit-Prediction")
...
def loadModel():
    ...
    try:
        ...
        model = tf.keras.models.load_model(os.path.join(os.getcwd(), "Models", "Hybrid_Digit_Recognition_Models.h5"))
        model_name = "Hybrid_Digit_Recognition_Models.h5"
    
    root.title("Digitera - Digit Prediction GUI - " + model_name) 
...
root = tk.Tk()
root.title("Digitera - Digit Prediction GUI") 
...
helpmenu.add_command(label="Guide", command = printGuide)
helpmenu.add_command(label="Info", command= printInfo)
